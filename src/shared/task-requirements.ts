...
const BROWSER_INTENT = /\b(browse|browser|click|navigate|open (?:the )?page|scroll|search (?:the )?(?:web|site)|use (?:the )?(?:tab|website)|read (?:the )?(?:transcript|captions))\b/i;
const DELIVERY_INTENT = /\b(push|create (?:a )?(?:pull request|pr)|merge (?:the )?(?:pull request|pr)|publish|send (?:an )?(?:email|message)|upload|download|purchase|buy|delete)\b/gi;
