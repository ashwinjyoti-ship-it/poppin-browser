...
export class BrowserAgentStateStore {
  ...
  async save(taskSpace: BrowserTaskSpace | null): Promise<void> {
    ...
    await writeFile(temporaryPath, `${JSON.stringify({ version: 1, taskSpace }, null, 2)}\n`, 'utf8');
...
export function normalizeTaskSpace(value: unknown): BrowserTaskSpace | null {
  ...
  if (record.version !== 1 || !record.taskSpace || typeof record.taskSpace !== 'object') return null;
  ...
  return { ...space, tabIds: [...space.tabIds] } as BrowserTaskSpace;
}
